...
Al módulo del compilador, encargado del  \textit{tokenizado} o análisis léxico se le suele llamar \textit{Lexer}.\\\\%\comentario{Yo más bien diría que es dividir la cadena de entrada en tokens, como dices en el siguiente parrafo. Fijate que si ya hubiera una división, no leeeriamos la entrada letra por letra.}
...
%\comentario{En general, yo recomiendo que en cada frase solo se introduzca un concepto nuevo, conectado con el concepto de la frase anterior. En las dos frases anteriores, pasa a explicar la transición, la devolución de los tipos y su relación con expresiones regulares.}
...
\end{itemize}
%\comentario{Varias cosas en la lista anterior, las palabras en ingles las ponías en letra itálica. En algunos casos hablas de detalles de la implementación y en los strings no hablas de si pueden contener saltos de linea, si pueden llevar caracteres escapados, que pasa con las comillas, etc. Tampoco entiendo la lógica de lo que sigue, que parece más de la parte de análisis sintáctico, aunque hay partes de semántica e implementación.}
\subsection{Lexer}
\noindent ¿Pero, cómo se distingue cada token, cómo se distingue un ``if`` de un  ``print``?, pues bien, en este caso haremos uso de la librería o mejor dicho, módulo de Python llamado SLY (Sly Lex Yacc). Esta librería que a su vez de dos herramientas muy conocidas llamadas lex y yacc (Yet Another Compiler Compiler), nos facilita el desarrollo de tanto lexers como parsers para el desarrollo de compiladores. Concretamente, nos facilita dos clases  \textit{Lexer} y \textit{Parser}, que nos harán más amigable el proceso de parsing y tokenizado del código fuente de nuestro lenguaje. \comentario{Quizás hacer una parte de herramientas, pero aquí no corresponde.} \\\\
...
\comentario{Aquí no viene a cuento.}
\\\\
...
), caracteres (p. ej. para distinguir entre el tipo de comparación), y por último números enteros o flotantes. Ahora veamos la estructura de nuestras instrucciones de código intermedio.
\[ 
